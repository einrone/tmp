...
class ImageContainer(FetchPath):
    def __init__(
        ...
        if self.files_to_extract:
            self.__create_structurize_data()
        else:
            self.structurized_data = self.list_of_path


        if exclude_files:
            for id_to_exclude in exclude_files:
                for files in self.structurized_data:
                    id_ = list(files.keys())[0]
                    if id_to_exclude == id_:
                        self.structurized_data.remove(files)
                    else:
                        pass
        else:
            pass                
    ...
    def __create_structurize_data(self) -> List:
        """
            ...
            if len(self.files_to_extract) > 1:
                for files in self.files_to_extract:
                    ...
                    if not temp_dict[patient_id][files]:
                        ...
                        assert patient_id == temp_dict[patient_id][files].parent.name
        ...
        if self.index < len(self.structurized_data):
            ...
            if len(self.files_to_extract) > 1:
                for mode, paths in image_path.items():
                    if id_value == paths.parent.name:
                        image_dict[id_value][mode] = self.load_image(paths)
                    else:
                        raise ValueError("Something went wrong the id value of the file and in the dictonary doesnt correspond")
                   
            else:
                if id_value == list(image_path.values())[0].parent.name:
...
if __name__ == "__main__":
    ...
    #for idx, img in enumerate(generate_img):
    #    print(idx)
