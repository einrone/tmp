...
class ModelPerformance:
    def __init__(
        ...
        plot_image_path, plot_metric_path, experiment_path = ModelPerformance.__experiments(model_name)
    ...
    def print_performance(self, phase, epoch, metric_dict) -> None:
        ...
        for metric_name, metric_value in metric_dict[phase].items():
            ...
            print("---------------------------------------------------------------------------------------------------------------------------")
            print(f"Phase: {phase} --- Current epoch {epoch}: {metric_name}, current value {metric_value[-1]}. Difference: {diff} --- {arrow}")
    ...
    def save_metrics(self, metrics_dict: dict, save_to_csv) -> None:
        ...
        fig, axs = plt.subplots(nrows = len(metric_keys), sharex = True, sharey = True)
        for idx, metric_name in enumerate(metric_keys):
            print(train_values[metric_name])
            length = np.arange(len(train_values[metric_name]))

            axs[idx].set_title(f"Median of {metric_name} vs epoch")
            axs[idx].plot(length, train_values[metric_name])
            axs[idx].plot(length, validation_values[metric_name])
            axs[idx].legend(["train", "validation"],loc="upper right")

        plt.tight_layout()
        plt.savefig(self.plot_metric_path / "metrics.png")
        plt.close()
